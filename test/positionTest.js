...
contract('Position', (accounts) => {
  ...
  before(async () => {
    ...
    fullPosition = utils.range(params.levels)
      .map((n) => ((n + 1) % params.slotCount) << (n * params.slotBits))
      .reduce(utils.sumReducer)
    childPosition = 1
    parentPosition = utils.range(params.levels - 1)
  ...
  describe('child()', async () => {
    it('Returns the child address', async () => {
      const result = await positionInstance.child.call(parentPosition, childPosition)
